...
@section('content')
    <div class="category-container">
        ...
        <div id="selectedCategory">{{ $product->chapter->name }} / {{ $product->category->name }}</div>
        <button id="cancelButton" style="display: none;">Отмена</button>
        <form id="productForm" method="POST" action="{{ route('product_update', $product->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="chapterId" id="chapterId" value="{{ $product->chapterId }}">
            ...
            <div class="add-product-param-block">
                <div class="add-product-param">
                    ...
                    @component('components.main-input', ['placeholder' => 'Введите название товара', 'type' => 'text', 'name' => 'name', 'value' => $product->name]) @endcomponent
                </div>
                <div class="add-product-param">
                    <div class="add-product-param-title">Цену товара</div>
                    @component('components.main-input', ['placeholder' => 'Введите цену', 'type' => 'number', 'name' => 'price', 'value' => $product->price]) @endcomponent
                </div>
                <div class="add-product-param">
                    <div class="add-product-param-title">Адресс проведения сделки</div>
                    @component('components.main-input', ['placeholder' => 'Введите адрес', 'type' => 'text', 'name' => 'address', 'value' => $product->address]) @endcomponent
                </div>
                <div class="add-product-param">
                    <div class="add-product-param-title">Оисание объявления</div>
                    @component('components.main-input', ['placeholder' => 'Введите описание', 'textarea' => true, 'name' => 'description', 'value' => $product->description]) @endcomponent
                </div>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <div class="toggle-button-group">
                    <input type="radio" name="type" id="new" value="0" {{ $product->type == 0 ? 'checked' : '' }}>
                    <label for="new" class="toggle-button">Новое</label>
                    <input type="radio" name="type" id="used" value="1" {{ $product->type == 1 ? 'checked' : '' }}>
                    <label for="used" class="toggle-button">Б/У</label>
                </div>
            </div>
            <div class="mb-3">
                <label>Метод связи</label><br>
                <input type="radio" name="connectMethod" value="1" {{ $product->connectMethod == 1 ? 'checked' : '' }}> Только чат
                <input type="radio" name="connectMethod" value="2" {{ $product->connectMethod == 2 ? 'checked' : '' }}> Только телефон
                <input type="radio" name="connectMethod" value="3" {{ $product->connectMethod == 3 ? 'checked' : '' }}> Оба
            </div>
...
<style>
    ...
    .category-block {
        display: none; /* Скрыть блок категорий по умолчанию */
        width: 750px;
    ...
    #productForm {
        display: block; /* Показать форму по умолчанию */
        margin-top: 20px;
        width: 80%;
    }
</style>
...
<script>
    ...
    function toggleVisibility(show) {
        ...
        categoryBlock.style.display = show ? 'flex' : 'none';
